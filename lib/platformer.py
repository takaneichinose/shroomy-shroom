...
class Platformer(Object):
  ...
  max_dy: float = 4
  bounce: float = 0
  x_direction: Direction | None = None
  y_direction: Direction | None = None
  is_walking: bool = False
  is_jumping: bool = False
  is_falling: bool = False
  is_ground: bool = True
  solid_tilemap: int = 0
  solid_list: list[tuple[int, int]] = []
  disabled_controls: bool = False
  collided_x: Direction | None = None
  collided_y: Direction | None = None
  ...
  def __check_solid(self, direction) -> bool:
    if direction == Direction.up:
      x1: float = self.x + self.margin.left
      x2: float = self.x + self.width - self.margin.right - 1
      y1: float = self.y + self.margin.top
      y2: float = self.y + self.margin.top + 1
    elif direction == Direction.down:
      x1: float = self.x + self.margin.left
      x2: float = self.x + self.width - self.margin.right - 1
      y1: float = self.y + self.height - self.margin.bottom - 1
      y2: float = self.y + self.height - self.margin.bottom
    elif direction == Direction.left:
      x1: float = self.x + self.margin.left
      x2: float = self.x + self.margin.left + 1
      y1: float = self.y + self.margin.top
      y2: float = self.y + self.height - self.margin.bottom - 1
    elif direction == Direction.right:
      x1: float = self.x + self.width - self.margin.right - 1
      x2: float = self.x + self.width - self.margin.right
      y1: float = self.y + self.margin.top
      y2: float = self.y + self.height - self.margin.bottom - 1
    else:
      return False
  ...
  def __update_x(self) -> None:
    is_left: bool = pyxel.btn(pyxel.KEY_LEFT) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_LEFT)
    is_right: bool = pyxel.btn(pyxel.KEY_RIGHT) or pyxel.btn(pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT)


    if is_left and not self.disabled_controls:
      self.dx -= self.acceleration
      self.x_direction = Direction.left
      self.is_walking = True
    elif is_right and not self.disabled_controls:
      self.dx += self.acceleration
    ...
    dx_direction: Direction = Direction.left if self.dx < 0 else Direction.right


    if self.__check_solid(dx_direction):
      self.dx = 0
      self.collided_x = self.x_direction
      tile_width: float = pyxel.floor(self.x / self.tile_width) * self.tile_width
  ...
  def __update_y(self) -> None:
    is_space: bool = pyxel.btnp(pyxel.KEY_SPACE) or pyxel.btnp(pyxel.GAMEPAD1_BUTTON_A)

    self.dy += self.gravity


    if is_space and self.is_ground and not self.disabled_controls:
    ...
    if self.__check_solid(self.y_direction):
      ...
      tile_height: float = pyxel.floor(self.y / self.tile_height) * self.tile_height


      if self.y_direction == Direction.up:
        self.y = tile_height + self.tile_height / 2 - self.margin.top
